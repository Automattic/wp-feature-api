...
async function initialize() {
	...
	// Tool definitions
	server.setRequestHandler(ListToolsRequestSchema, withLogging('ListTools', async () => {
		...
		return {
			tools: features.map((tool) => {
				...
				properties = Object.entries( (tool.input_schema?.properties || {}) ).reduce(
					(
						acc: {
							[ key: string ]: {
								type: string;
	...
	// Tool handlers
	server.setRequestHandler(CallToolRequestSchema, withLogging('CallTool', async (request: { params: { name: string; arguments: any } }) => {
